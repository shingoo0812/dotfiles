...
if (Test-Path $scoopShims) {
    if (-not ($env:PATH -split ';' -contains $scoopShims)) {
        $env:PATH += ";$scoopShims"
    }
}
if (Test-Path $scoopApps) {
    $scoopBin = Get-ChildItem "$scoopApps\*\current\bin" -ErrorAction SilentlyContinue | Select-Object -ExpandProperty FullName
    foreach ($bin in $scoopBin) {
        if (-not ($env:PATH -split ';' -contains $bin)) {
            $env:PATH += ";$bin"
        }
    }
}
# ユーザー環境変数にScoopを永続的に追加
$currentUserPath = [Environment]::GetEnvironmentVariable("PATH", "User")
$scoopShims = "$env:USERPROFILE\scoop\shims"

if (-not ($currentUserPath -split ';' -contains $scoopShims)) {
    $newPath = "$currentUserPath;$scoopShims"
    [Environment]::SetEnvironmentVariable("PATH", $newPath, "User")
    Write-Host "Scoopを環境変数に永続的に追加しました"
}
#########################oh-my-posh end###############################
...
if (Test-Path $aliasPath) {
    try {
        . $aliasPath
        Write-Host "エイリアスファイルが読み込まれました" -ForegroundColor Green
    } catch {
        Write-Warning "エイリアスファイルの読み込みでエラー: $($_.Exception.Message)"
    }
} else {
...
$env:MSSdk = "1"
$env:MAGICK_CODER_MODULE_PATH="F:\Program Files\ImageMagick-7.1.1-Q16-HDRI\modules\coders" 
# Neovim用の環境変数設定
$env:TERM = "xterm-256color"
$env:COLORTERM = "truecolor"
# Windows TerminalにカレントディレクトリをOSC 7で通知
# WezTermとWindows Terminalの両方に対応したカレントディレクトリ通知
# WezTermとWindows Terminalの両方に対応したカレントディレクトリ通知
function Global:Set-CurrentDirectoryForWindowsTerminal {
    $loc = (Get-Location).Path
    # Windows Terminal用 (OSC 9;9)
    Write-Host -NoNewline "`e]9;9;`"$loc`"`e\"
    # WezTerm用 (OSC 7)
    $escapedLoc = $loc -replace '\\', '/' -replace ' ', '%20'
    Write-Host -NoNewline "`e]7;file://localhost/$escapedLoc`e\"
}
# プロンプトが表示されるたびに実行
if (-not $global:__originalPrompt) {
    $global:__originalPrompt = Get-Content Function:\prompt
}
$null = New-Item -Path Function:\global:prompt -Value {
    Set-CurrentDirectoryForWindowsTerminal
